...
const DepartmentKHR = () => {
  ...
  const getDepartmentType = (name: string) => {
    ...
    if (lowerName.includes('it') || lowerName.includes('tech') || lowerName.includes('development')) return 'Technology';
    if (lowerName.includes('finance') || lowerName.includes('accounting') || lowerName.includes('budget')) return 'Finance';
    if (lowerName.includes('sales') || lowerName.includes('marketing') || lowerName.includes('business')) return 'Sales & Marketing';
    if (lowerName.includes('operation') || lowerName.includes('admin') || lowerName.includes('management')) return 'Operations';
    if (lowerName.includes('legal') || lowerName.includes('compliance')) return 'Legal & Compliance';
    if (lowerName.includes('support') || lowerName.includes('service') || lowerName.includes('help')) return 'Support Services';
    return 'General';
  };

  const getBusinessType = (name: string) => {
    const lowerName = name.toLowerCase();
    if (lowerName.includes('core') || lowerName.includes('main') || lowerName.includes('primary')) return 'Core Business';
    if (lowerName.includes('support') || lowerName.includes('auxiliary') || lowerName.includes('service')) return 'Support Functions';
    if (lowerName.includes('strategic') || lowerName.includes('planning') || lowerName.includes('executive')) return 'Strategic';
    if (lowerName.includes('operational') || lowerName.includes('production') || lowerName.includes('delivery')) return 'Operational';
    return 'Standard';
  };
  ...
  const groupDataByField = (data: Department[], field: string): GroupedData[] => {
    ...
    const grouped = data.reduce((acc: any, item) => {
      ...
      switch (field) {
        ...
        case 'business_type':
          ...
          break;
        case 'manager_status':
          groupKey = item.Department_Head === 'No Manager' ? 'Without Manager' : 'With Manager';
          break;
        case 'name_length':
  ...
  const renderGroupedTable = () => {
    if (groupBy === 'none') {
      return <DatatableKHR data={data} columns={columns} selection={true} textKey="Department_Name" />;
    }
          ...
          >
            ...
            >
              <div className="d-flex align-items-center">
                ...
                <span className="badge badge-primary ms-2">{group.count} departments</span>
              </div>
                ...
                <div className="d-flex gap-3">
                  ...
                  {groupBy === 'name_length' && (
                    <small className="text-info">
                      ...
                      Avg Length: <strong>{Math.round(group.items.reduce((sum, item) => sum + item.Department_Name.length, 0) / group.count)} chars</strong>
                    </small>
                  )}
                  {groupBy === 'manager_status' && (
                    <small className="text-success">
                      <i className="ti ti-user-check me-1"></i>
                      With Manager: <strong>{group.items.filter(item => item.Department_Head !== 'No Manager').length}</strong>
                    </small>
                  )}
                  ...
                  <small className="text-muted">
                    ...
                    Sample: <strong>{group.items[0]?.Department_Name.substring(0, 15)}{group.items[0]?.Department_Name.length > 15 ? '...' : ''}</strong>
                  </small>
                </div>
              </div>
            </div>
                  ...
                  {groupBy !== 'none' && (
                    <div className="alert alert-info m-3 mb-0 d-flex justify-content-between align-items-center">
                      <div>
                        ...
                        <span className="ms-2">
                          ({groupedData.length} groups, {data.length} total departments)
                        </span>
                      </div>
