...
const AddEditAttendancePolicyModal: React.FC<Props> = ({ onSuccess, data }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (target instanceof HTMLInputElement && target.type === 'checkbox') {
      setFormData((prev: any) => ({ ...prev, [name]: (target as HTMLInputElement).checked }));
      return;
    }
      ...
      <div className="modal-dialog modal-dialog-centered modal-lg">
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">{data ? "Edit Accrual Plan" : "Add Accrual Plan"}</h5>
              ...
              <div className="row">
                <div className="col-md-12 mb-3">
                  ...
                  {validated && !(formData.name) && (
                    <span className="text-danger small">Required — please enter name</span>
                  )}
                </div>

                <div className="col-md-6 mb-3">
                  <label className="form-label">Accrued Gain Time</label>
                  <select name="accrued_gain_time" className="form-select" value={formData.accrued_gain_time ?? ""} onChange={handleChange} required>
                    <option value="">Select</option>
                    <option value="start_of_accrual">At the start of the accrual period</option>
                    <option value="end_of_accrual">At the end of the accrual period</option>
                  </select>
                  {validated && !(formData.accrued_gain_time) && (
                    <span className="text-danger small">Required — select accrued gain time</span>
                  )}
                </div>

                <div className="col-md-6 mb-3">
                  <label className="form-label">Carry-Over Time</label>
                  <select name="carry_over_time" className="form-select" value={formData.carry_over_time ?? ""} onChange={handleChange} required>
                    <option value="">Select</option>
                    <option value="start_of_year">At the start of the year</option>
                    <option value="allocation_date">At the allocation date</option>
                    <option value="other">Other</option>
                  </select>
                  {validated && !(formData.carry_over_time) && (
                    <span className="text-danger small">Required — select carry-over time</span>
                  )}
                </div>

                <div className="col-md-6 mb-3 d-flex align-items-center">
                  <div className="form-check">
                    <input className="form-check-input" type="checkbox" id="basedOnWorked" name="based_on_worked_time" checked={!!formData.based_on_worked_time} onChange={handleChange} />
                    <label className="form-check-label ms-2" htmlFor="basedOnWorked">Based on worked time?</label>
                  </div>
                </div>
