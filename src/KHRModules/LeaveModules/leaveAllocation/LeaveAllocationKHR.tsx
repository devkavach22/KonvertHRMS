    ...
    try {
      ...
      const mappedData: AttendancePolicyType[] = safeResult.map((item: any) => {
        ...
        if (item.employee && (item.employee.name || item.employee.full_name)) {
          ...
          employee_name = empMap[String(item.employee_id)] ?? String(item.employee_id);
        }

        const leaveTypeLabel = item.holiday_status_name ?? item.leave_type_name ?? item.leave_type ?? (item.holiday_status_id ? String(item.holiday_status_id) : "");

        return {
          id: String(item.id ?? item.allocation_id ?? item._id ?? item.employee_id ?? Math.random()),
          key: String(item.id ?? item.allocation_id ?? item._id ?? item.employee_id ?? Math.random()),
          employee_name,
