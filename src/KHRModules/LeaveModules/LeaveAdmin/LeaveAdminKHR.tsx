...
// --- Types based on your new API structure ---
type TabType =
...
const LeaveAdminKHR = () => {
  ...
  const MOCK_API_RESPONSE: DashboardResponse = {
    success: true,
    dashboard: {
      present_today: 2,
      planned_leaves: 2,
    ...
    tables: {
      ...
      absent_unplanned: [
        {
          employee_id: 103,
          employee_name: "Keval Tilavat",
          leave_type: "Unplanned Absence",
          department: "Engineering",
          last_attendance: "2025-12-30T18:00:00",
          from: "2026-01-03",
          to: "2026-01-03",
          no_of_days: 1,
          status: "Absent",
  ...
  const fetchData = async () => {
    ...
    setTimeout(() => {
      // In real scenario: const response = await getLeaveDashboard();
      setDashboardData(MOCK_API_RESPONSE);
      setLoading(false);
    }, 500);
  };
  ...
  // --- Helper to map Component Tab IDs to API Response Keys ---
  const tabToKeyMapping = {
    total_present_employee: "present_today",
    planned_leaves: "planned_leaves",
    unplanned_leaves: "absent_unplanned",
  ...
  const getCurrentTableData = () => {
    ...
    const apiKey = tabToKeyMapping[activeTab];
    // Typescript cast to ensure key access
    const rawData = dashboardData.tables[apiKey as keyof TableData] || [];
