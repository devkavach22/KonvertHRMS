...
const getUserRole = () => {
  if (typeof window !== "undefined") {
    return localStorage.getItem("user_role") || ROLES.EMPLOYEE;
  }
...
const filterMenu = (menuItems: any[], role: string) => {
  return menuItems.reduce((acc, item) => {
    // Check if the item is restricted for this role
    ...
    if (newItem.submenuItems && newItem.submenuItems.length > 0) {
      newItem.submenuItems = filterMenu(newItem.submenuItems, role);

      // Remove parent if all children are hidden
      if (
        newItem.submenuItems.length === 0 &&
        (newItem.submenu || newItem.tittle)
      ) {
        return acc;
      }
    }
...
// 5. Export the Filtered Data directly
export const SidebarDataTest = filterMenu(rawSidebarData, currentUserRole);
